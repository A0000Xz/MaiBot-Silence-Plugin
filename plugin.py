...
class SilencePlugin(BasePlugin):
    ...
    "permissions": {
        "white_or_black_list": ConfigField(
            type=str,
            default="whitelist",
            description="管理员用户列表的类型，支持'whitelist'（白名单）和'blacklist'（黑名单）两种模式",
    ...
    "experimental": {
        "silence_expression_learning": ConfigField(
            type=bool,
            default=False,
            description="是否在沉默状态下继续进行表达方式学习"
        ),
        "silence_special_check": ConfigField(
            type=bool,
            default=False,
            description="启用针对特定用户或群聊的默认沉默功能（实验性功能）"
        ),
        "silence_someone_list": ConfigField(
            type=list,
            default=[123456789],
            description="被沉默检查的用户ID列表，仅在启用默认沉默功能时生效",
            item_type="number",
        ),
        "silence_group_list": ConfigField(
            type=list,
            default=[123456789],
            description="被沉默检查的群聊ID列表，仅在启用默认沉默功能时生效",
            item_type="number",
        )
    }
}
